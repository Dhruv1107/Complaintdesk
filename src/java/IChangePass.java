...
public class IChangePass extends HttpServlet {
                    ...
                    if(p2.equals(p3))
                    {
                        if(password.equals(p1))
                        {
                            ...
                            Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/cd?zeroDateTimeBehavior=convertToNull","root","");
                             PreparedStatement ps=con.prepareStatement("update incharge set pwd= ? where usr=?");
                             ps.setString(1,p2);
                             ps.setString(2,username);
                               int i= ps.executeUpdate();
                               if(i>0)
                                {
                                RequestDispatcher rd=request.getRequestDispatcher("isettings.html");  
